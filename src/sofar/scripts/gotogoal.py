...
class Gotogoal(Node):
    ...
    def __init__(self):
        ...
        self.front_clearance = 0.7  # [m]
        self.side_clearance = 0.5  # [m]
        self.desired_angle_window_front = 0.6 # [rad] = 34.4[deg] angle window for front scan 
        self.desired_angle_window_side = 0.3 # [rad] = 17.2[deg] angle window for side scan 
        ...
        # Goals 
        self.goals = [(1.0, 0.0), (1.0, 0.5), (1.5, -1)]
...
# -------------------------------------------------------------- #
    ...
    def compute_goal_following(self, out_msg, distance_to_goal, angle_diff):
        ...
        linear_x = np.clip(linear_x, 0.0, self.max_linear_speed)    # max linear speed
...
# -------------------------------------------------------------- #
    ...
    def callback_navigate(self, in_msg):
        ...
        if not self.following_wall and (self.obstacle_detected or self.obstacle_on_right) :
            self.following_wall = True
            self.get_logger().info("Obstacle detected: switching to wall-following.")

        elif self.following_wall and not self.obstacle_detected and not self.obstacle_on_right:
            self.following_wall = False
...
# -------------------------------------------------------------- #
    ...
    def callback_scan(self, scan_msg):
        ...
        window_front = round(self.desired_angle_window_front/scan_msg.angle_increment) # = 34
        window_right = round(self.desired_angle_window_side/scan_msg.angle_increment) # = 17
        
        front_indices_1 = slice(idx_front, idx_front+window_front+1) # [0, 34]
        front_indices_2 = slice(idx_front-window_front+num_rays, idx_front+num_rays) # [-34, 0]+360 = [343, 359]
        right_indices = slice(idx_right-window_right+num_rays, idx_right+window_right+num_rays+1) # [-107, -74] + 360 = [253, 288] 
